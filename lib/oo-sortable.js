    ...
    if(el) {
      ...
      if(this.data.groupName){
        for (var i = items.length - 1; i >= 0; i--) {
          var query = {
            sortOrder: $(items[i]).index() + 1
          }
          if (self.data.fieldName)
            query[self.data.fieldName] = $(items[i]).closest(".js-sortGroup").attr('id')
          else
            query.group = $(items[i]).closest(".js-sortGroup").attr('id')

          Collection[collection].update({_id: items[i].id}, {$set: query});
        }
      } else {
        for (var i = items.length - 1; i >= 0; i--) {
